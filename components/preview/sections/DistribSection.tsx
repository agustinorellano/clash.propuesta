...
const CHANNELS = [
  ...
  {
    key: '4', label: 'Campañas digitales',
...
const ACC_ITEMS = [
  {
    key: '0', highlighted: true,
    title: 'Link único', sub: 'Un solo enlace para todo',
  ...
  },
  {
    ...
    title: 'Códigos QR', sub: 'Acceso simple y directo',
  ...
  },
  {
    ...
    title: 'Stickers físicos', sub: 'Producción y entrega incluida',
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 48 }}>
        <EditableText sectionId={SID} contentKey="eyebrow" defaultValue="Distribución" tag="span"
          style={{ fontSize: 10, fontWeight: 700, color: '#dc2626', letterSpacing: '0.25em', textTransform: 'uppercase' }} />
        <EditableText sectionId={SID} contentKey="title" defaultValue="Llevá los beneficios a todos los puntos de contacto." tag="h2"
          style={{ fontSize: 44, fontWeight: 900, color: '#f1f1f1', marginTop: 16, lineHeight: 1.08, letterSpacing: '-1px' }} />
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {CHANNELS.map(({ key, label, icon }) => (
              <div key={key} style={{
                ...
                {icon}
                <EditableText sectionId={SID} contentKey={`channel.${key}`} defaultValue={label} tag="span"
                  style={{ color: 'rgba(255,255,255,0.78)', fontSize: 14, fontWeight: 500 }} />
              </div>
            ))}
          </div>
        </div>

        {/* Columna derecha — A través de */}
        <div>
          <div style={{
            fontSize: 10, fontWeight: 700, textTransform: 'uppercase',
            letterSpacing: '1.5px', color: 'rgba(255,255,255,0.35)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {ACC_ITEMS.map((item) => (
              <div key={item.key} style={{
                background: 'rgba(255,255,255,0.05)',
                border: `1px solid ${item.highlighted ? 'rgba(220,38,38,0.4)' : 'rgba(255,255,255,0.09)'}`,
                borderRadius: 13,
                overflow: 'hidden',
              }}>
                {/* Head */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 20px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <EditableText sectionId={SID} contentKey={`acc.${item.key}.title`} defaultValue={item.title} tag="div"
                      style={{ color: '#fff', fontSize: 15, fontWeight: 700 }} />
                    <EditableText sectionId={SID} contentKey={`acc.${item.key}.sub`} defaultValue={item.sub} tag="div"
                      style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginTop: 2 }} />
                  </div>
                </div>
                {/* Body */}
                <div style={{ padding: '0 20px 18px 74px' }}>
                  <EditableText sectionId={SID} contentKey={`acc.${item.key}.body`} defaultValue={item.body} multiline tag="p"
                    style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, lineHeight: 1.65 }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{
        ...
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
          <EditableText sectionId={SID} contentKey="footer.strong" defaultValue="Todos los canales sincronizados:" tag="strong"
            style={{ color: 'rgba(255,255,255,0.75)', fontWeight: 700 }} />{' '}
          <EditableText sectionId={SID} contentKey="footer.body" defaultValue="cualquier cambio en el panel se refleja en tiempo real en todos los puntos de distribución." tag="span"
